"""Executive-level report generator with risk scoring."""
...
# Maps severity + requires_auth to an estimated CVSS 4.0 base score
SEVERITY_TO_CVSS = {
    ("critical", False): 9.3,
    ("critical", True): 8.7,
    ("high", False): 8.1,
...
class EliteReportGenerator:
    """Generate elite bug bounty reports with intelligence annotations."""
    ...
    def generate_elite_reports(
        ...
        """
        Generate elite reports for all submission-ready findings.
        
        Returns list of generated file paths.
        """
        generated = []

        # Only generate reports for findings worth submitting
        reportable = [
            f for f in findings
        ...
        if not reportable:
            # Generate a summary-only report explaining why nothing is worth submitting
    ...
    def _generate_markdown_report(
        ...
        """Generate a comprehensive markdown report for one finding."""
    ...
    def _generate_field_report(
        ...
        """Generate platform-specific field format for quick submission."""
    ...
    def _h1_format(
        ...
        """HackerOne submission field format."""
        return (
            f"{warning_block}"
            f"FIELD: Title\n"
            f"{title}\n\n"
    ...
    def _bugcrowd_format(
        ...
        """Bugcrowd submission field format."""
        return (
            f"{warning_block}"
            f"FIELD: Title\n"
            f"{title}\n\n"
    ...
    def _generate_master_summary(
        ...
        """Generate a master summary of the entire hunt."""
        ...
        # Skipped findings (for transparency)
    ...
    def _generate_summary_only(
        ...
        """Generate a summary when no findings are worth submitting."""
        total = len(all_findings)
    ...
    def _build_cvss4_vector(self, finding: dict) -> str:
        """Build a CVSS 4.0 vector string from finding attributes."""
        ...
        pr = "L" if requires_auth else "N"  # Privileges Required
